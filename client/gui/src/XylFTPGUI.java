...
public class XylFTPGUI extends javax.swing.JFrame implements XylFTPInterface {
	...
	try {
	    ...
	    JOptionPane.showMessageDialog(this,
		    "There is an error, the command is: "
		    + ex.GetCommand() + " , the error message is: "
		    + ex.GetMessage() + " please disconnect and connect again!",
		    "XylFTP cwd error", JOptionPane.ERROR_MESSAGE);
	    if(FTPConnection.GetStatus() == 0) {
		try {
	...
	for(int i = 0; i < TmpRemoteFileList.length; i++) {
	    ...
	    if(listEntries[i].charAt(k) == 'd' || listEntries[i].charAt(k) == 'l') { //whether it is a directory
		...
		if(listEntries[i].charAt(k) == ' ') {
		    ...
		    if(delimCount == 4) { //read file size
	    ...
	    m = listEntries[i].lastIndexOf(" ->"); //deal with symbolic link
	    if(m == -1) { //assign file name
	...
	int echoMeaning;
	try {
	    if(FTPConnection.IsPassive()) {  //passive mode
		FTPConnection.SendCommand("PASV\r\n");
		ShowCommand("PASV\r\n");
		echo = FTPConnection.GetEcho(); //Get the echo for PASV
		echoMeaning = ProcessEcho(echo);
		if(echoMeaning == 5) {
		...
		echo = FTPConnection.GetEcho(); //Get the echo for LIST (Beginning data transferring)
		if (!IsValidEcho(echo))
		    throw new XylFTPException("ls", FTPConnection.GetStatus(), "Invaild echo.");
		echoMeaning = ProcessEcho(echo);
		if(echoMeaning == 1) {
		    strRemoteFileList = FTPConnection.GetListPassive();
		    ParseRemoteFileList(strRemoteFileList);
		    echo = FTPConnection.GetEcho(); //Get the echo for LIST (Finished data transferring)
		    echoMeaning = ProcessEcho(echo);
		}
	    } else {  //active mode
		...
		public int getRowCount() { return Math.max(RemoteFileList.length, TABLE_MIN_ROWS);}
	...
	if(!(localFile.exists() && localFile.canRead())) {
	    JOptionPane.showMessageDialog(this,
		    "The file does not exist or cannot read!"
		    , "XylFTP upload error", JOptionPane.ERROR_MESSAGE);
	    return;
	}
	...
	try {
	    ...
	    echo = FTPConnection.GetEcho();
	    if (!IsValidEcho(echo))
		throw new XylFTPException("put", FTPConnection.GetStatus(), "Invaild echo.");
	    echoMeaning = ProcessEcho(echo);
	    if(echoMeaning == 1) {
	...
	if(FTPConnection.GetStatus() != 2 || RemoteFileList == null) {
	    JOptionPane.showMessageDialog(this,
		    "You have not connected to server!"
		    , "XylFTP download error", JOptionPane.ERROR_MESSAGE);
	    return;
	}
	...
	JFileChooser chooserSave = new JFileChooser();
	//dlgSave = new FileDialog(this, "Where do you save the file", FileDialog.SAVE);
	...
	File localFile = chooserSave.getSelectedFile();
	if(localFile.exists()) {
	    int iChoice = JOptionPane.showConfirmDialog(this,
		    "The file already exists, do you want to overwrite it?",
		    "XylFTP download Message", JOptionPane.YES_NO_OPTION);
	    if(iChoice == 1) {
		return;
	    }
	}
	...
	if(FTPConnection.GetStatus() != 0) { //if connection already exists, first break it
	    try {
		FTPConnection.CloseConnection();
	    } catch (Exception e) {
	    }
	...
	try {
	    ...
	    JOptionPane.showMessageDialog(this,
		    "There is an error when establishing control connection with the server, the error message is: "
		    + ex.getMessage()
		    , "XylFTP connection error", JOptionPane.ERROR_MESSAGE);
	    try {
		FTPConnection.CloseConnection();
	    } catch (Exception e) {
	    }
	...
	try {
	    ...
	    JOptionPane.showMessageDialog(this,
		    "Login failed!", "XylFTP connection error", JOptionPane.ERROR_MESSAGE);
	    try {
		FTPConnection.CloseConnection();
	    } catch (Exception e) {
	    }
	...
	try {
	    ...
	    JOptionPane.showMessageDialog(this,
		    "There is an error listing remote files, the command is: "
		    + ex.GetCommand() + " , the error message is: "
		    + ex.GetMessage() + " please connect again!",
		    "XylFTP connection error", JOptionPane.ERROR_MESSAGE);
	    try {
		FTPConnection.CloseConnection();
	    } catch (Exception e) {
	    ...
	    JOptionPane.showMessageDialog(this,
		    "There is an error listing remote files"
		    + " , the error message is: " +ex.getMessage()
	...
	TableModel emptyModel = new AbstractTableModel() {
	    public int getColumnCount() { return 3; }
	    public int getRowCount() { return TABLE_MIN_ROWS;}
	    public boolean isCellEditable(int row,int col){return false;}
	...
	if(FTPConnection.GetStatus() != 2 || RemoteFileList == null) {
	    JOptionPane.showMessageDialog(this,
		    "You have not connected to server!"
		    , "XylFTP delete error", JOptionPane.ERROR_MESSAGE);
	    return;
	}
	int selectedRow = jTableRemoteFiles.getSelectedRow();
	if(selectedRow == -1 || selectedRow >= RemoteFileList.length) {
	...
	int iChoice = JOptionPane.showConfirmDialog(this,
		...
		+ (RemoteFileList[selectedRow].IsDirectory ? "Folder " : "File ")
		+ strSel + "? (Remember that you can only delete empty folders)",
		"XylFTP download Message", JOptionPane.YES_NO_OPTION);
	if(iChoice == 1) {
	    return;
	}
	...
	if(FTPConnection.GetStatus() != 2 || RemoteFileList == null) {
	    JOptionPane.showMessageDialog(this,
		    "You have not connected to server!"
		    , "XylFTP makedir error", JOptionPane.ERROR_MESSAGE);
	    return;
	}
	...
	if(FTPConnection.GetStatus() != 2 || RemoteFileList == null) {
	    JOptionPane.showMessageDialog(this,
		    "You have not connected to server!"
		    , "XylFTP rename error", JOptionPane.ERROR_MESSAGE);
	    return;
	}
	int selectedRow = jTableRemoteFiles.getSelectedRow();
	if(selectedRow == -1 || selectedRow >= RemoteFileList.length) {
    ...
    private void initComponents() {
        ...
        jBtnConnect.setFont(new java.awt.Font("宋体", 1, 12));
        ...
        jBtnDisconnect.setFont(new java.awt.Font("宋体", 1, 12));
        ...
        jBtnUpload.setFont(new java.awt.Font("宋体", 1, 12));
        ...
        jBtnDownload.setFont(new java.awt.Font("宋体", 1, 12));
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addComponent(jTextServer, javax.swing.GroupLayout.PREFERRED_SIZE, 183, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(jLabelUser))
                    .addComponent(jToolBarViews, javax.swing.GroupLayout.PREFERRED_SIZE, 106, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jTextFieldUser, javax.swing.GroupLayout.PREFERRED_SIZE, 84, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jTextPort, javax.swing.GroupLayout.PREFERRED_SIZE, 38, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jCkbMode, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addContainerGap())
        );
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jLabelServer)
                    .addComponent(jTextServer, javax.swing.GroupLayout.PREFERRED_SIZE, 21, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jLabelUser)
                    ...
                    .addComponent(jCkbMode))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addComponent(jToolBarViews, javax.swing.GroupLayout.PREFERRED_SIZE, 25, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap())
        );
        ...
        jTableRemoteFiles.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jTextRemotePWD, javax.swing.GroupLayout.DEFAULT_SIZE, 728, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jButtonCdUp, javax.swing.GroupLayout.PREFERRED_SIZE, 25, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap())
        );
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jTextRemotePWD, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jButtonCdUp, javax.swing.GroupLayout.PREFERRED_SIZE, 26, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void jMenuItemAboutActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItemAboutActionPerformed
        JOptionPane.showMessageDialog(this,
                    "Copyright (C) XiYou Linux Group."
                    , "About", JOptionPane.PLAIN_MESSAGE);
    }//GEN-LAST:event_jMenuItemAboutActionPerformed

    private void jButtonCdUpActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonCdUpActionPerformed
        Cdup();
      
       

    }//GEN-LAST:event_jButtonCdUpActionPerformed
...
// TODO 将在此处添加您的处理代码：
    ...
    private void jTableRemoteFilesMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTableRemoteFilesMouseClicked
	int clickCount = 0;
	clickCount = evt.getClickCount() ; // 连续击打次数为2,则说明是双击了鼠标
	if(clickCount==2) {
	    if(FTPConnection.GetStatus() != 2 || RemoteFileList == null) {
		JOptionPane.showMessageDialog(this,
			"You have not connected to server!"
    ...
    }//GEN-LAST:event_jTableRemoteFilesMouseClicked
    
    private void jTextRemotePWDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jTextRemotePWDActionPerformed
	ChangeRemotePWD(jTextRemotePWD.getText());
    }//GEN-LAST:event_jTextRemotePWDActionPerformed
    ...
    private void jCkbModeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCkbModeActionPerformed
// TODO 将在此处添加您的处理代码：
    ...
    }//GEN-LAST:event_jCkbModeActionPerformed
...
// TODO 将在此处添加您的处理代码：
    ...
    private void jMenuItemConnectMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jMenuItemConnectMouseClicked
// TODO 将在此处添加您的处理代码：
    }//GEN-LAST:event_jMenuItemConnectMouseClicked
    
    private void jMenuItemMakedirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItemMakedirActionPerformed
	MakeDir();
    }//GEN-LAST:event_jMenuItemMakedirActionPerformed
    ...
    private void jMenuItemDisconActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItemDisconActionPerformed
	Disconnect();
    }//GEN-LAST:event_jMenuItemDisconActionPerformed
    ...
    private void jMenuFilesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuFilesActionPerformed
// TODO 将在此处添加您的处理代码：
    }//GEN-LAST:event_jMenuFilesActionPerformed
