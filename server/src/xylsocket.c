...
int xyl_listen(short port) 
{
	...
	if ((bind(socket_fd,(struct sockaddr *) &server_addr,sizeof(server_addr)) == -1)) {
		...
		if (errno == EADDRINUSE) {
			...
			perror("sock_fd is not a valid descriptor\n");
		}
		else if (errno == EACCES) {
...
int xyl_accept(int socket_fd)
{
	...
	while (1) {
		...
		if (connect_fd == -1) {
			fprintf(stderr,"accept error errno is %d",errno);
	...
	while (1) {
		...
		if (ret == -1) {
			...
			if (errno == ENETUNREACH) {
				perror("Network is unreachable.\n");
			}	
			else if (errno == ETIMEDOUT) {
				perror("Timeout while attempting connection\n");
			}
			else if (errno == EISCONN) {
				perror("The socket is already connected.\n");
			}
